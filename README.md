...
NETRA is not limited to its interactive terminal interface. The core scanner can be imported and used directly in external Python scripts or automation workflows.

### Generating the Standard Report

You can run a scan and generate the exact same formatted report displayed by the CLI using `format_scan_report`:

```python
from netra import PortScanner, format_scan_report
...
1. **`PortScanner`** performs the network scan.
2. **`ScanResultSet`** contains the raw, structured results.
3. **`format_scan_report()`** produces the same human-readable report used by the CLI.

### Programmatic Access to Structured Results

If you are building custom tooling or data pipelines, you can bypass text formatting and work directly with the structured `ScanResult` models:
...
This demonstrates that NETRA can be used both as:
1. A ready-made scanner/report utility
2. A reusable scanning library
